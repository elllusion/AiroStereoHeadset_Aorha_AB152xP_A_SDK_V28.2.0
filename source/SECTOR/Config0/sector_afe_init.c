 ...
 *
 * sector_afe_init.c defines the initialization sequence to setup AFE.
 *
...
/////////////////////////////////
//#define AB1520S_5x5_1v5_Diff
#define AB1520S_6x6_3v_LRVCM
/////////////////////////////////
...
{
	{
	    ...
	    //- MCU clock source=48M
			...
			{0x1D, 0x10, 0x00},
				...
				#endif
			#endif
			{0x1E, 0x63, 0x01}, //sw_osc_freq_lv:10 1Mhz,buck_out_lv:011 1v25
			#ifdef BLUETOOTH_SPEAKER
	...
	//- Audio
		...
		{0xA5, 0xA8, 0x00}, //1-mic + ADC 2-order, ear_drv=1b'0
			...
			#ifdef BLUETOOTH_SPEAKER
				#ifdef SCO_WOOFER
					{0xA6, 0x80, 0x08}, //Audio-in and Audio-out off, Line-in off, mic_bias_en=0, auadc_ref_sel=2b'00 (Differentail 3.0V)
				#else
					{0xA6, 0x80, 0x0B}, //Audio-in and Audio-out off, Line-in off, mic_bias_en=0, auadc_ref_sel=2b'00 (LRVCM 3.0V)
				#endif
				{0xA7, 0xA7, 0x6B}, // MIC bias 2.7V for AB1520
			#else
				{0xA6, 0x80, 0x00}, //Audio-in and Audio-out off, Line-in off, mic_bias_en=0, auadc_ref_sel=2b'00 (Differentail 2.0)
	...
	#endif
	        //- Audio end
	        {0xAB, 0x2A, 0x1F}, // Steve ADC0 enable all channel -> Minga change enable channel Vin , Temp and Vbat
	        {0xAC, 0x00, 0x00},
		{0xFF, 0x03, 0x8C},
	        {0xAD, 0x1F, 0xFF},
	        {0xAE, 0x00, 0x00},
	    ...
	    //- XTAL offset MCU clock source=32k
	        ...
	        {0x40, 0x30, 0x01},
	        {0xB4, 0x0F, 0x27}, //{0xB4, 0x2F, 0x85}, //If the register is changed, the follow 0xB4 shall also be changed!!!
	        {0xB5, 0x59, 0x59},
		...
		{0x36, 0xFA, 0xBE}, //set fsys2ctrltop_en=0 (gating), set fsys2mcu_en=0  (gating), dummy off
			...
			{0x2D, 0x08, 0xF7}, //Buck_always_on, xo_always_on, disable force 500KHz, set clk32k_xo_div1=0111, set fsys2dco32k_ctrl_en=1  (can't gating)
	        #elif defined(EXTERNAL_XO32K)
	        ...
	        {0xB4, 0x0F, 0x27}, //{0xB4, 0x2F, 0x85}, //set pass_xo2pll_en=1
			/* RF Calibration Workaround */
			{0x36, 0xFA, 0xBF},//{0x36, 0x7A, 0xBF},//{0x36, 0xFA, 0xBF}, //set fsys2ctrltop_en=1, set fsys2mcu_en=1, dummy off
	        {0xC2, 0x73, 0x44}, //Tx on
	        {0xC2, 0x71, 0x01}, //Control by MCU
	        {0xFF, 0xFF, 0x00}, //AFE Init wait for 0 format
	        {0xC2, 0x46, 0x01}, //Bit Mask for 0xC2/70 bit0
			...
			{0x2D, 0x08, 0x27},//{0x2D, 0x08, 0xF7},//{0x2D, 0x08, 0xB7}, //disable xo_always_on, disable force 500KHz, set fsys2dco32k_ctrl_en=1
            {0xFF, 0x03, 0xE8}, //wait for 1ms
				 ...
				 {0x1E, 0x53, 0x01}, //sw_osc_freq_lv:01 1Mhz,buck_out_lv:001 1v25
						...
						#else
						{0x1E, 0x63, 0x00}, //sw_osc_freq_lv:10 1Mhz,buck_out_lv:001 1v2
						#endif
			   #endif
...
{
	{
	    ...
	    //- XTAL offset MCU clock source=32k
            ...
            #ifdef AB1528C_ONLY //AB1528P
                #ifdef BLUETOOTH_SPEAKER
                {0xB4, 0x0D, 0x27}, //{0xB4, 0x2F, 0x85}, //If the register is changed, the follow 0xB4 shall also be changed!!!
	              {0xB5, 0x59, 0x59},
                #else //headset
                {0xB4, 0x0F, 0x47}, //{0xB4, 0x2F, 0x85}, //If the register is changed, the follow 0xB4 shall also be changed!!!
	              {0xB5, 0x6C, 0x6C},
                #endif
            #else  //AB152XP
                #ifdef BLUETOOTH_SPEAKER
...
{
	{
...

	    {
			...
			{0x29, 0x43, 0x0C}, //cv_initial_dac_value<9:0>780
			{0x2C, 0x00, 0x00},//40
			{0x2D, 0x18, 0xF7}, //xo_always_on, buck_always_on, set clk32k_xo_en=1, force 500KHz, set clk32k_xo_div1=0111
			{0x2E, 0x00, 0x00},
	    ...
	    //------------------------
	        ...
	        #ifdef BLUETOOTH_SPEAKER
				//For Music Box application (32k to FM chip)
				{0x2D, 0x08, 0xF7}, //xo_always_on, disable force 500KHz, set clk32k_xo_div1=0111, set fsys2dco32k_ctrl_en=1  (can't gating)
				{0xB4, 0x0D, 0x27}, //{0xB4, 0x2D, 0x85}, //set pass_xo2pll_en=0 (gating)
				{0xBF, 0x0F, 0x5B}, //Set complete_rdy, switch clock to divide_from_xo
	        #else
				//For other applications (Headset and External_XO32K)
